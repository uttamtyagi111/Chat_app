...
(function () {
    ...
    // Function to determine API URLs based on current environment
    function getApiConfig() {
        ...
        if (isLocalhost) {
            return {
                ...
                uploadUrl: "http://localhost:8000/chat/user-chat/upload-file/"
            };
        } else {
            return {
                apiUrl: "https://www.chat.wishgeekstechserve.com/api/chat/user-chat/",
                wsUrl: "wss://www.chat.wishgeekstechserve.com/api/ws/chat/",
                uploadUrl: "https://www.chat.wishgeekstechserve.com/api/chat/user-chat/upload-file/"
            };
        }
    }
...
function injectStyles() {
    ...
    style.innerHTML = `
        ...
        /* Environment indicator (optional - for debugging) */
        .env-indicator {
            position: absolute;
            top: 5px;
            right: 10px;
    ...
    async function uploadFile(file, widgetId, uploadUrl) {
    ...
    function handleFileAttachment(inputElement, socket, widgetId, chatMessages, uploadUrl) {
        inputElement.addEventListener("change", async () => {
            ...
            try {
                ...
                // Upload file to S3
                const uploadResult = await uploadFile(file, widgetId, uploadUrl);
    ...
    function main() {
        ...
        const WIDGET_CONFIG = getApiConfig();
        console.log("🌍 Environment detected:", window.location.hostname);
        console.log("🔗 API Config:", WIDGET_CONFIG);
        ...
        chatHeader.innerHTML = `
            Chat with Us
            <div class="env-indicator">${window.location.hostname.includes('localhost') ? 'DEV' : 'PROD'}</div>
        `;
        ...
        function connectWebSocket(roomId) {
            ...
            socket.onopen = () => {
                console.log(`${CHAT_LOG_PREFIX} 🔌 WebSocket connected to:`, WIDGET_CONFIG.wsUrl);
                // Initialize file attachment handling with upload functionality
                handleFileAttachment(fileInput, socket, widgetId, chatMessages, WIDGET_CONFIG.uploadUrl);
            };
            socket.onclose = (e) => {
