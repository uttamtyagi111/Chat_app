...
document.addEventListener("DOMContentLoaded", async function () {
	...
	// const widgetContainer = document.createElement("div");
	// widgetContainer.id = "chat-widget";
	// ADD these functions before the widgetContainer creation:
	async function loadCSS() {
		const cssResponse = await fetch('../../static/css/chat_widget.css');
	...
	async function loadHTML() {
		const htmlResponse = await fetch('../../static/html/chat-widget.html');
		const htmlText = await htmlResponse.text();

		// Process HTML with dynamic values
		return htmlText
