...
async function initializeChatWidget() {
    try {
        ...
        const baseApi = isLocal ? "http://localhost:8000" : "https://chat.wishgeekstechserve.com/api";
        ...
        async function loadCSS() {
            const cssResponse = await fetch(WIDGET_CONFIG.cssUrl);
        ...
        async function loadHTML() {
            const htmlResponse = await fetch(WIDGET_CONFIG.htmlUrl);
        ...
        function handleIncomingMessage(data) {
            ...
            try {
                ...
                // *** MAIN MESSAGE HANDLING - CRITICAL FIX ***
                // Handle regular messages (including User messages from server echo)
                if (data.message || data.file_url) {
                    ...
                    // CRITICAL FIX: Check if this is a server echo of user's own message
                    const isUserMessageEcho = data.sender === "User" && sentMessages[data.message_id];

                    if (isUserMessageEcho) {
                        console.log("🔄 Server echo of user message, updating status only:", data.message_id);
                        // Update the existing message status instead of creating duplicate
                        updateMessageStatus(data.message_id, data.status || "delivered", data.message, data.file_url, data.file_name);
                        return;
                    }
                    ...
                    console.log(`📝 Displaying NEW message from ${data.sender} with class: ${messageClass}`);
